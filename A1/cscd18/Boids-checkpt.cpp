...
//    Min and max values ------------|--|
    ...
    glui->add_separator();
      ...
      GLUI_Spinner *cat_spinner
          = glui->add_spinner("cat's leadership strongness: c_rule", GLUI_SPINNER_FLOAT, &c_rule);
      //rotation_spinner->set_speed(5.0);
      cat_spinner->set_float_limits(0, 1, GLUI_LIMIT_CLAMP);

      GLUI_Spinner *witch_spinner
          = glui->add_spinner("witch's scariness strongness: w_rule", GLUI_SPINNER_FLOAT, &w_rule);
      //rotation_spinner->set_speed(5.0);
...
//     // for (int j=0; j<nBoids; j++) {
//     //     if (sqrt(pow(Boid_Location[j][0] - onMouseCoordinates, 2)
//     //                 + pow(Boid_Location[j][1] - Boid_Location[i][1], 2)
//     //                 + pow(Boid_Location[j][2] - Boid_Location[i][2], 2)) 
//     //             <= 15) {
//     //         num_of_boids += 1;
